...
Use `--theme-revision` to pin the theme checkout to a branch, tag, or commit. The built-in Even theme defaults to `terav2`. AstroPaper and custom themes use the repository's default branch unless a revision is supplied.
    ...
    steps:
      ...
      - name: Generate markdown
        run: |
          ...
          gh release download $ZOLA_VERSION --repo getzola/zola -p '*x86_64-unknown-linux-gnu*' -O- | tar -xz -C /tmp && mv /tmp/zola /usr/local/bin
